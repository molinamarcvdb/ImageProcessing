...
%% Read and gray
Img=imread('images/Original_4_b4.jpg');
...
%% Binaritzar 
Img_b= im2uint8(imbinarize(histeq(Img)));
SE=strel('disk',7);
Img_b=imclose(Img_b,SE);
figure(3);
imshow(Img_b); title('Imatge Binaritzada')
%% Extreure elements petits
Img_bw = bwareaopen(Img_b, 50);
labeledImage = bwlabel(Img_bw);
...
SE=strel('sphere', 10)
Mask=im2uint8(imdilate(biggestBlob, SE));
figure(4); imshow(Mask); title('Màscara sense etiqueta')
...
phi(340:350,10:20)=-c0;
phi(10:40,160:190)=-c0;
phi(10:40,10:40)=-c0;
figure(8);
imagesc(phi);
...
for k=1:maxiter
    ...
    if mod(k,50)==1
     ...
     frame = getframe(h);
        ...
        if frm == 1        
            imwrite(imind,cm,'MAMseg4.gif','gif', 'Loopcount',inf);
        else        
            imwrite(imind,cm,'MAMseg4.gif','gif','WriteMode','append');
        end
...
figure(11);
subplot(121); imagesc(maskedImage,[0, 255]); axis off; axis equal; colormap(gray); hold on;  contour(phi, [0,0], 'r');
msg=['phi result , iteration number=' num2str(k)]; title(msg);
subplot(122); imshow(phi); axis off; hold off;
...
figure(12);subplot(131);imshow(maskedImage); title('Original Image'); axis off; hold off
