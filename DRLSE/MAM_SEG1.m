...
%% Read and gray
Img=imread('images/Original_1_b1.jpg');
...
Img_G=conv2(Img,G,'same');  % smooth image by Gaussiin convolution
figure(3);
...
for k=1:maxiter
    ...
    if mod(k,50)==1
        ...
        if frm == 1        
            imwrite(imind,cm,'mam.gif','gif', 'Loopcount',inf);
        else        
            imwrite(imind,cm,'mam.gif','gif','WriteMode','append');
        end
...
%% Get area
SE = strel('sphere',20);
Region = imopen(phi, SE);
Region = im2uint8(imcomplement(Region));
Im_mask =imadjust(im2uint8(Img));
Im_mask(~Region) = 0;
Im_mask=localcontrast(Im_mask);
figure(7); subplot(131); imshow(Img);title('Original Image'); axis off; hold off
